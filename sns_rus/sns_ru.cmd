...
set RoadL=Локация
set FilesL
set TranslateL=SNS-DOS переведён на 10%
...
echo   -=Pre-release=-
echo -=Stable Release=-
echo -=Released: 05.13.2022=-
echo [WARNING!] This version has 20 experimental features.
echo 10 of 20 features are now stable!
...
if "%SnsCode%"=="SnsInstallBonus" (echo You got 1000 SnsCoins code! & echo Code: V5SA-CM3Q-ODEM-55FK)
if "%SnsCode%"=="SnsFilledGrill" (echo You got gift code! & echo Code: M56K-V309-3C95-CRT1)
if "%SnsCode%"=="SnsInBeta" (goto InBeta)
if "%SnsCode%"=="SnsAir" (echo You got 1000 SnsCoins code! & echo Code: X48F-556T-FWB9-ZS76)
...
echo Хотите запустить SNS-DOS 0.6.0?
echo Configuration:
...
echo Хотите запустить SNS-DOS 0.7.0?
echo Конфигурация:
echo CPU: Custom 199 MHz Pentium CPU with 2 MB RAM
echo GPU: Custom 3 MB GF GPU
echo Storage: 6 MB drive
...
:vmbios
echo [SnsVM BIOS Setup v1.00b3 x32]
echo CPU: Intel Core 0000 199 MHz
...
echo Exit or quit - выключить SNS-DOS 0.7.0
echo About - Версия и сборка
echo cmd and taskbar - запуск программ
echo time, news, run, start - Другое 
set command=
set /p command=%name%: 
if "%command%"=="clear" (
cls
...
if "%command%"=="run" (
echo SNS-DOS может запускать файлы.
echo Введите расположение папки и имя файла для открытия.
set runapp_folder=
...
echo SnsVM with SNS-DOS up tp 0.7.5
echo SnsFiles
echo SnsUI
echo Critical
echo SnsCode
